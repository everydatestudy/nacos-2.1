...
public class DistroConsistencyServiceImpl implements EphemeralConsistencyService, DistroDataProcessor {
	 ...
	 */
	/**
	 * Check sum when receive checksums request. Service检查服务，用于从其他节点更新已变更的Service
	 * 
	 * @param checksumMap map of checksum 某个服务对应的checksum
	 * @param server      source server request checksum checksum请求的来源
	 */
	//当前的方法主要处理服务的3种场景：不需要处理的、需要更新的、需要删除的。
			...
			try {
				...
				if (Loggers.DISTRO.isDebugEnabled()) {
					Loggers.DISTRO.debug(
							"[NACOS-DISTRO] datum change notified, key: {}, listener count: {}, action: {}", datumKey,
							count, action.name());
				}
			} catch (Throwable e) {
				Loggers.DISTRO.error("[NACOS-DISTRO] Error while handling notifying task", e);
