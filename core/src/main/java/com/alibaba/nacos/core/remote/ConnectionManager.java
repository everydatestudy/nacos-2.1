 ...
 */
@Service
//在Nacos 2.x之后，使用Grpc协议代替了http协议，长连接会保持客户端和服务端发送的状态，
//在源码中ConnectionManager 管理所有客户端的长连接。
//ConnectionManager每3秒检测所有超过20S内没有发生过通讯的客户端，
//向客户端发起ClientDetectionRequest探测请求，
//如果客户端在指定时间内成功响应，则检测通过，否则执行unregister方法移除Connection。
public class ConnectionManager extends Subscriber<ConnectionLimitRuleChangeEvent> {
	...
	public boolean traced(String clientIp) {
		return connectionLimitRule != null && connectionLimitRule.getMonitorIpList() != null
				&& connectionLimitRule.getMonitorIpList().contains(clientIp);
	}
	...
	public synchronized boolean register(String connectionId, Connection connection) {
		...
		if (connection.isConnected()) {
			...
			Loggers.REMOTE_DIGEST.info("new connection registered successfully, connectionId = {},connection={} ",
	...
	private boolean checkLimit(Connection connection) {
		...
		if (connectionLimitRule != null) {
			...
			if (StringUtils.isNotBlank(appName)
					&& connectionLimitRule.getCountLimitPerClientApp().containsKey(appName)) {
			...
			public void run() {
				try {
					...
					Loggers.REMOTE_DIGEST.info(
							"Total count ={}, sdkCount={},clusterCount={}, currentLimit={}, toExpelCount={}",
					...
					Loggers.REMOTE_DIGEST.info("Check over limit for ip limit rule, over limit ip count={}",
							expelForIp.size());
					...
					if (expelCount > 0) {
						for (Map.Entry<String, Connection> entry : entries) {
							Connection client = entry.getValue();
							if (!expelForIp.containsKey(client.getMetaInfo().clientIp)
									&& client.getMetaInfo().isSdkSource() && expelCount > 0) {
								expelClient.add(client.getMetaInfo().getConnectionId());
								expelCount--;
					...
					for (String expelledClientId : expelClient) {
						try {
							...
							Loggers.REMOTE_DIGEST.error("Error occurs when expel connection, expelledClientId:{}",
									expelledClientId, e);
						}
					}
					...
					if (CollectionUtils.isNotEmpty(outDatedConnections)) {
						...
						for (String outDateConnectionId : outDatedConnections) {
							try {
								...
								if (connection != null) {
									...
									Loggers.REMOTE_DIGEST.info("[{}]send connection active request ",
											outDateConnectionId);
								} else {
									latch.countDown();
								}
						...
						Loggers.REMOTE_DIGEST.info("Out dated connection check successCount={}",
						...
						for (String outDateConnectionId : outDatedConnections) {
							...
							if (!successConnections.contains(outDateConnectionId)) {
								Loggers.REMOTE_DIGEST.info("[{}]Unregister Out dated connection....",
										outDateConnectionId);
								// key5:注销掉
