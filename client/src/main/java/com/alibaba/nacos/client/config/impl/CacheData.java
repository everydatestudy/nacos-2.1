...
public class CacheData {
	...
	/**
	 * Add listener if CacheData already set new content, Listener should init
	 * lastCallMd5 by CacheData.md5
	 *
	...
	/**
	 * Returns the iterator on the listener list, read-only. It is guaranteed not to
	 * return NULL.
	 */
	...
	private void safeNotifyListener(final String dataId, final String group, final String content, final String type,
		...
		Runnable job = () -> {
			...
			try {
				...
				// Before executing the callback, set the thread classloader to the classloader
				// of
				// the specific webapp to avoid exceptions or misuses when calling the spi
				// interface in
				// the callback method (this problem occurs only in multi-application
				// deployment).
		...
		try {   // 设置了自己监听器的异步线程池，就用自己的执行回调job
	...
	/**
	 * 1.first add listener.default is false;need to check. 2.receive config change
	 * notify,set false;need to check. 3.last listener is remove,set to false;need
	 * to check
	 *
